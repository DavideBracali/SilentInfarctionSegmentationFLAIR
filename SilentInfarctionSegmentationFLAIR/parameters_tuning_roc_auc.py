...
def process_patient(args_tuple):
    data, alpha, beta, extend_dilation_radius, n_std, \
    ...
    for g, gamma in enumerate(gammas):
        ...
        metrics = evaluate_voxel_wise(ref_mask, gt)

        stats.loc[g, "gamma"] = gamma
        stats.loc[g, "TPF"] = metrics["vw-TPF"]
    ...
    fpf = stats_sorted["FPF"].values
    tpf = stats_sorted["TPF"].values
    auc_1p = np.trapezoid(tpf, fpf)
...
def main(data_folder, results_folder, init_points, n_iter, n_cores):
    ...
    def auc_obj(alpha, beta, extend_dilation_radius, n_std,
        ...
        for chunk in range(n_chunks):
            ...
            args_list = [
                (chunk_data[patient], alpha, beta, extend_dilation_radius,
            ...
            stats_list.extend([r[0] for r in results])  # to compute global AUC
            auc_list.extend([r[1] for r in results])    # AUCs of individual images
        ...
        for stats in stats_list:
                pos += stats["P"].values
                neg += stats["N"].values
                tp += stats["TPF"].values * stats["P"].values
                fp += stats["FPF"].values * stats["N"].values
        tpf_global = tp / pos
        fpf_global = fp / neg
        sort_idx = np.argsort(fpf_global)
        fpf_sorted = np.concatenate(([0], fpf_global[sort_idx], [1]))
        tpf_sorted = np.concatenate(([0], tpf_global[sort_idx], [1]))
        auc_global = np.trapezoid(tpf_sorted, fpf_sorted)
        ...
        os.makedirs(os.path.join(results_folder, "training_ROCs"), exist_ok=True)
        plt.savefig(os.path.join(results_folder, "training_ROCs", f"ROC_{iteration}.png"))
        plt.close()

        print(f"Elapsed time: {(time.time()-start):.2f} s")
        return auc_global, auc_list

    def auc_obj_global(*args, **kwargs):
        global tr_auc_list
        tr_auc_global, tr_auc_list = auc_obj(*args,
                                train_patients=tr_patients,
                                data=preloaded_data,
                                **kwargs)
        return tr_auc_global
